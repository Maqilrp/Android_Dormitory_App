...
fun AirminumScreen(navController: NavController, userViewModel: UserViewModel = viewModel()) {
    // data store
...
//                    }
        ...
        AirminumItem(modifier = Modifier.padding(paddingValues), navController)
    }
}

@Composable
fun AirminumItem(modifier: Modifier = Modifier, navController: NavController) {
    ...
    var jumlahGalon by remember { mutableStateOf(TextFieldValue("")) }
    ...
    Column(
        ...
        OutlinedTextField(
            value = jumlahGalon,
            onValueChange = { jumlahGalon = it },
            leadingIcon = { Icon(imageVector = Icons.Filled.LocalDrink, contentDescription = "nim") },
...
fun AirminumPreview() {
    DormitoryAppTheme {
        AirminumScreen(rememberNavController())
    }
}